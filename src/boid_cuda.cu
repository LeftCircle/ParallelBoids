    ...
    if (idx < num_boids) {
        ...
        for (int i = 0; i < num_boids; ++i) {
            ...
            if (dist_sq > 0 && dist_sq < neighbor_distance_sq) {
                ...
                separation_force = separation_force + (diff / dist / dist); // F = diff / dist^2 (normalized diff / dist)
        ...
        if (neighbor_count > 0) {
            ...
            separation_force = normalize(separation_force) * max_speed; // Desired velocity
            ...
            alignment_sum = normalize(alignment_sum) * max_speed; // Desired velocity
            ...
            desired_cohesion = normalize(desired_cohesion) * max_speed; // Desired velocity
